        ...
        for line in f:
            ...
            if "<A " in line.upper():
                ...
                if href_match and title_match:
                    ...
                    # Allow duplicates here; global dedupe will happen later
...
def merge_folders_global(f1, f2):
    merged = defaultdict(dict)
    seen_urls = set()  # global set of URLs we've already added anywhere

    # Helper to add from one source with global dedupe
    def add_from_source(source_folders):
        for path, links in source_folders.items():
            for url, title in links.items():
                if url in seen_urls:
                    # We've already placed this URL somewhere else; skip it
                    continue
                merged[path][url] = title
                seen_urls.add(url)

    # Order matters: first file "wins" for which folder gets the URL
    add_from_source(f1)
    add_from_source(f2)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python merge_bookmarks_global.py bookmarks1.html bookmarks2.html merged.html")
        sys.exit(1)
    ...
    print("Merging with global URL dedupe...")
    merged = merge_folders_global(folders1, folders2)
